  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-dark-950/80 backdrop-blur-md border-b border-dark-800/50'
          : 'bg-transparent'
      }`}
    >
      <nav className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        <button
          ...
          className="text-lg font-semibold text-dark-100 hover:text-accent-400 transition-colors"
          ...
          <ul className="px-6 py-4 flex flex-col gap-4">
            {NAV_LINKS.map((link) => (
              <li key={link.id}>
                <button
                  onClick={() => scrollToSection(link.id)}
                  className="text-dark-200 hover:text-accent-400 transition-colors w-full text-left"
                >
                  {link.label}
                </button>
              </li>
